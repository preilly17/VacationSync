...
export default function Home() {
  ...
  return (
    <div className="dashboard-canvas min-h-screen">
      <div className="dashboard-container w-full pb-24 pt-24 sm:pt-28">
        <div className="dashboard-stack">
          <section
            aria-labelledby="dashboard-hero"
            className="dashboard-hero border border-white/15 p-8 text-white shadow-xl backdrop-blur-lg sm:p-12"
            style={heroStyles}
          >
            <div className="grid gap-6">
              <span className="eyebrow">Your travel hub</span>
              <div className="space-y-4">
                <h1 id="dashboard-hero" className="h-hero">
                  Dashboard
                </h1>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Button
                  ...
                  className="dashboard-button-primary rounded-full px-6 py-2 text-base font-semibold"
                >
                  Plan a New Trip
                </Button>
                {nextTripChip ? (
                  <span className="dashboard-chip px-3 py-1 text-xs font-medium text-white/90">
                    {nextTripChip}
                  </span>
                ) : null}
              </div>
          ...
          <section
            ...
            className="dashboard-stats-band"
          >
            ...
            {isLoading ? (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {Array.from({ length: 4 }).map((_, index) => (
                  <Card
                    ...
                    className="dashboard-card rounded-2xl border border-slate-100/60 bg-white/80 p-6"
                  >
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <HighlightCard
                  icon={<Plane className="h-5 w-5 text-[var(--brand-1)]" aria-hidden="true" />}
                  value={highlightStats.upcoming}
                  label="Upcoming trips"
                  href={primaryTrip ? `/trip/${primaryTrip.id}` : "/"}
                />
                <HighlightCard
                  icon={<Globe2 className="h-5 w-5 text-[var(--brand-1)]" aria-hidden="true" />}
                  value={highlightStats.destinations}
                  label="Destinations"
                  href={primaryTrip ? `/trip/${primaryTrip.id}` : "/"}
                />
                <HighlightCard
                  icon={<CalendarDays className="h-5 w-5 text-[var(--brand-1)]" aria-hidden="true" />}
                  value={highlightStats.daysToNext ?? "—"}
                  label="Days to next trip"
                  href={primaryTrip ? `/trip/${primaryTrip.id}` : "/"}
                />
                <HighlightCard
                  icon={<UserRound className="h-5 w-5 text-[var(--brand-1)]" aria-hidden="true" />}
          ...
          <section className="grid gap-12 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
            <div className="flex flex-col gap-12">
              <section aria-labelledby="upcoming-trips-heading" className="space-y-6">
                <div className="dashboard-section-heading">
                  ...
                  <div>
                    <h2 id="upcoming-trips-heading" className="section-title text-2xl text-slate-900">
                      Upcoming trips
                    </h2>
                ...
                {error ? (
                  <Card className="dashboard-card rounded-3xl border border-amber-200 bg-amber-50/80 p-6 text-amber-900">
                ...
                {isLoading ? (
                  <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
                    {Array.from({ length: 3 }).map((_, index) => (
                      <Card
                        ...
                        className="dashboard-card overflow-hidden rounded-3xl border border-slate-100 bg-white p-4"
                      >
                  ...
                  <Card className="dashboard-card flex flex-col items-center justify-center gap-5 rounded-3xl border border-dashed border-slate-200 bg-white p-10 text-center">
                    ...
                    <Button
                      onClick={handlePlanTrip}
                      className="dashboard-button-primary rounded-full px-6 py-2 text-white"
                    >
                      Plan a New Trip
                    </Button>
                  </Card>
              ...
              {insights.length > 0 ? (
                <section aria-labelledby="insights-heading" className="space-y-6">
                  <div className="dashboard-section-heading">
                    <span className="section-eyebrow">Keep momentum</span>
                    <div>
                      <h2 id="insights-heading" className="section-title text-2xl text-slate-900">
                        Helpful insights
                      </h2>
                      <div className="section-divider" />
                    </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {insights.map((insight) => (
                      <Link key={insight.id} href={insight.href} className="group block h-full">
                        <Card
                          data-variant="insight"
                          className="dashboard-card flex h-full flex-col justify-between overflow-hidden rounded-3xl p-6 transition-transform group-hover:-translate-y-1"
                        >
                          ...
                          <span className="dashboard-insight-link mt-6 text-sm font-semibold">Go to trip →</span>
                        </Card>
                      </Link>
                    ))}
                  </div>
            ...
            <aside className="flex flex-col gap-6 self-start">
              <section aria-labelledby="converter-heading">
                <Card className="dashboard-card dashboard-card--accent dashboard-currency-card overflow-hidden rounded-3xl">
                  <header>
                    <div className="flex items-center gap-2">
                      <Globe2 className="h-5 w-5 text-[var(--brand-1)]" aria-hidden="true" />
                      <h2 id="converter-heading">Currency converter</h2>
                    </div>
                    <p>Quick conversions for the road.</p>
                  </header>
              ...
              {primaryTrip ? (
                <Card data-variant="next-trip" className="dashboard-card rounded-3xl p-6">
...
function HighlightCard({ icon, value, label, href }: HighlightCardProps) {
  return (
    <Link href={href} className="group block h-full">
      <Card className="dashboard-card dashboard-card--accent flex h-full flex-col justify-between gap-4 overflow-hidden rounded-3xl p-6 transition-transform group-hover:-translate-y-1">
        <div className="flex items-center justify-between">
          <div className="dashboard-icon-pill">
            {icon}
          </div>
          <span className="dashboard-stat-label text-xs">
            View
          </span>
        </div>
        <div>
...
function TripCard({ trip, onOpen }: TripCardProps) {
  ...
  useEffect(() => {
    const frame = requestAnimationFrame(() => setProgress(trip.progressPercent));
    return () => cancelAnimationFrame(frame);
  }, [trip.progressPercent]);

  return (
    <Card className="dashboard-card group flex h-full flex-col overflow-hidden rounded-3xl transition-transform hover:-translate-y-1">
      <div className="dashboard-trip-media relative aspect-video overflow-hidden">
        <img
          src={imageUrl}
          alt=""
        ...
        <div className="absolute bottom-3 left-4 right-4 flex flex-wrap items-center gap-2 text-xs font-medium text-white">
          <Badge className="dashboard-chip px-3 py-1 text-xs font-medium text-slate-800 dark:text-slate-100">
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Badge variant="secondary" className="dashboard-chip px-3 py-1 text-xs font-medium text-slate-700 dark:text-slate-100">
        ...
        <Button
          onClick={onOpen}
          className="dashboard-button-primary mt-auto rounded-full"
        >
          Open trip
        </Button>
      </div>
